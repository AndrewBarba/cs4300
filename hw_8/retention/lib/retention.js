...
class Retention {
  ...
  constructor(period, sections, init, then) {
    this._data = {};
  ...
  initDateQuery(period, section) {
  	...
  	let now = getDate();
  	let lt = getNextDate(now, -(section + 0));
  	let gte = getNextDate(now, -(section + 1));
  ...
  thenDateQuery(date, period, section) {
    ...
    let gte = getNextDate(date, section);
  	let lt = getNextDate(date, section + 1);
